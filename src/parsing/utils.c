...
/*   Updated: 2025/06/18 19:51:36 by abchaman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 		// 	{
// 		// 		char *temp = our_strjoin(cmd->collector, tokens[j - 1], get_from_env(cmd, env ,&str[i]));
// 		// 		tokens[j - 1] = temp;
// 		// 	}
// 		// 	else
...
// 			if (str[i + 1] == str[i])
// 				len = 2;
// 			tokens[j++] = our_strndup(cmd->collector, &str[i], len);
// 			i += len;
// 		}
// 		else if (str[i] == '|')
// 		{
// 			cmd->quote_flags[j] = 0;
// 			if (str[i + 1] != str[i])
// 				tokens[j++] = our_strndup(cmd->collector, &str[i], 1);
// 			i++;
// 		}
// 		else
// 		{
...
// 				if (j > 0 && (cmd->quote_flags[j - 1] == 1 || cmd->quote_flags[j - 1] == 2) && is_space == 0)
// 				{
// 					char *temp = our_strjoin(cmd->collector, tokens[j - 1] , our_strndup(cmd->collector, &str[start], i - start));
// 					tokens[j - 1] = temp;
// 				}
// 				else
// 					tokens[j++] = our_strndup(cmd->collector, &str[start], i - start);
// 			}
// 		}
// 	}
// 	tokens[j] = NULL;
